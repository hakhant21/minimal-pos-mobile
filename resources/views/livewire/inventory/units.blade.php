<div class="space-y-6">
    <div class="flex items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-xl font-bold text-gray-900 dark:text-white">{{ __('Units of Measure') }}</h2>
        </div>
    ...
    <div class="space-y-3">
        ...
        <div
            ...
            <p class="mt-3 text-sm font-medium text-gray-500 dark:text-gray-400">
                @if(!empty($search))
                {{ __('No units found matching') }} "{{ $search }}"
                @else
                {{ __('No units yet. Create units like Bottle, Can, Piece etc.') }}
                @endif
            </p>
        </div>
        @endforelse
    ...
    <div class="flex justify-center pt-2">
        <button wire:click="loadMore" wire:loading.attr="disabled"
            ...
            <span wire:loading.remove wire:target="loadMore">{{ __('Load More Units') }}</span>
        ...
        <div class="relative mx-4 w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-800">
            ...
            <div class="mt-4 text-center">
                ...
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    {{ __('Units used by product variants cannot be deleted.') }}
                </p>
            </div>
